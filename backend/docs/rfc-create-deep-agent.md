...
`DeerFlowClient` 是唯一公开 API。`create_deep_agent` 和 `make_lead_agent` 都是内部实现。
...
| `features` | `AgentFeatures` | 替换内置 middleware 实现 |
...
client = DeerFlowClient(features=AgentFeatures(auto_title=MyTitleMiddleware()))
...
### 3.2 `create_deep_agent` — 内部工厂（不公开）
...
### 3.3 `AgentFeatures` — 内置 Middleware 替换
...
client = DeerFlowClient(features=AgentFeatures(memory=MyMemoryMiddleware()))

# 两者组合 — config 参数给默认 middleware，但 title 换实现
client = DeerFlowClient(
    config={"memory": {"max_facts": 50}},
    features=AgentFeatures(auto_title=MyTitleMiddleware()),
)
```
...
def _assemble_from_features(feat: AgentFeatures, config: AppConfig) -> tuple[list, list]:
    chain = []
    extra_tools = []
...
```
Phase 1（当前 PR #1203）:
  ✓ 新增 create_deep_agent + AgentFeatures（内部 API）
...
Phase 2（#1380）:
  ...
  - make_lead_agent 改为薄壳调 create_deep_agent
...
| create_deep_agent | 内部实现，不公开 | 用户不需要接触 CompiledStateGraph |
| 配置覆盖 | `config` dict，和 config.yaml 结构一致 | 无新概念，deep merge 覆盖 |
| middleware 替换 | `features=AgentFeatures(memory=MyMW())` | bool 开关 + 实例替换 |
...
**设计原则**：
- `AgentFeatures`、`@Next/@Prev`、排序机制只作用于**主 agent**
